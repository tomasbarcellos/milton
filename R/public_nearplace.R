...
nearplace <- function(coords,targets,coord_names=c("lon","lat"),great_circ=T,
                      target_label="names"){
  require(purrr)
  require(sp)
  ...
  nearestplace <- purrr::map(.f = function(x) spDistsN1(pt=x %>% unlist, # Dist: coord vs. targets
                                                   pts=f_targets,longlat=great_circ) %>% 
                               which.min %>% # min value
